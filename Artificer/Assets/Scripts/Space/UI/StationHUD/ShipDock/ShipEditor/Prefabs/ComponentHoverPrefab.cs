...
namespace Space.UI.Station.Prefabs
{
    ...
    /// </summary>
    public class ComponentHoverPrefab : MonoBehaviour 
    {
        #region ATTRIBUTES
        ...
        public void DisplayComp(ComponentListener Con, Texture Tex)
        {
            ...
            float maxAngle = Mathf.Max(Icon.texture.width, Icon.texture.height);

            if (maxAngle > 75)
            {
                float sizeScale = 75f / maxAngle;

                Icon.transform.localScale = new Vector3(sizeScale, sizeScale, 1);
            }
