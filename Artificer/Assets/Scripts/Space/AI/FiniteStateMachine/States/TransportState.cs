...
public class TransportState : FSMState
{
    private Vector3 _dest;
    private Transform _follow;
    
    public TransportState(ShipMessegeController m, Transform npc, Transform follow)
    {
        _messege = m;
    ...
    public override void Act(List<Transform> enemies, Transform npc)
    {
        ...
        if (Vector3.Distance(npc.position, _dest) < 5.0f)
        {
            _messege.ReleaseKey(Control_Config.GetKey("moveUp", "ship"));
            //_dest = FindNewPoint(npc);
        }
